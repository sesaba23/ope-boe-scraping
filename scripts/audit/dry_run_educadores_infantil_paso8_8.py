"""Comprueba el conjunto cerrado de Educador Infantil frente al flujo efectivo."""
...
AUDITORIA = Path("informes/normalizacion_puestos/fase8_paso8_educadores_infantil.json")
SALIDA = Path("informes/normalizacion_puestos/fase8_paso8_educadores_infantil_dry_run.json")


def ejecutar(ruta_bd="datos/boe.db", auditoria=AUDITORIA, salida=SALIDA):
    informe_auditoria = json.loads(Path(auditoria).read_text(encoding="utf-8"))
    esperado = {int(k): v for k, v in informe_auditoria["conjunto_seguro"]["mapa_id_canon"].items()}
    ...
    resultado = {
        ...
        "capturas_paso7": [], "plazas_esperadas": informe_auditoria["conjunto_seguro"]["plazas"],
        "plan": plan,
    }
