...
def test_hash_archivo_y_leer_metadata_preservan_contrato(tmp_path):
    ruta_archivo = tmp_path / "contenido.bin"
    ruta_archivo.write_bytes(b"contenido conocido")
    assert base_datos.hash_archivo(ruta_archivo) == hashlib.sha256(b"contenido conocido").hexdigest()

    conexion = base_datos.conectar(tmp_path / "metadata.db")
    base_datos.crear_esquema(conexion)
    base_datos.guardar_metadata(conexion, schema_version=6, data_version=27)
    conexion.execute("INSERT INTO metadata(clave,valor) VALUES (?,?)", ("adicional", "valor"))

    metadata = base_datos.leer_metadata(conexion)
    assert metadata["schema_version"] == "6"
    assert metadata["data_version"] == "27"
    assert metadata["adicional"] == "valor"
    assert metadata.get("ausente") is None
    conexion.close()


def test_leer_metadata_propaga_ausencia_de_tabla(tmp_path):
    conexion = base_datos.conectar(tmp_path / "sin_metadata.db")
    with pytest.raises(sqlite3.OperationalError):
        base_datos.leer_metadata(conexion)
    conexion.close()


def test_copiar_sqlite_consistente_crea_snapshot_autonomo_desde_wal(tmp_path):
    origen = tmp_path / "origen.db"
    conexion = base_datos.conectar(origen)
    assert conexion.execute("PRAGMA journal_mode=WAL").fetchone()[0].lower() == "wal"
    base_datos.crear_esquema(conexion)
    base_datos.guardar_metadata(conexion, schema_version=6, data_version=27)
    conexion.execute("INSERT INTO metadata(clave,valor) VALUES (?,?)", ("origen", "wal"))
    conexion.commit()
    assert (tmp_path / "origen.db-wal").exists()

    destino = tmp_path / "copia.db"
    base_datos.copiar_sqlite_consistente(origen, destino)
    assert base_datos.leer_metadata(conexion)["origen"] == "wal"
    assert (tmp_path / "origen.db-wal").exists()

    copia = base_datos.conectar(destino, readonly=True)
    try:
        assert base_datos.leer_metadata(copia)["origen"] == "wal"
        assert base_datos.integrity_check(copia) == ["ok"]
        assert base_datos.foreign_key_check(copia) == []
    finally:
        copia.close()
        conexion.close()
    assert not (tmp_path / "copia.db-wal").exists()
    assert not (tmp_path / "copia.db-shm").exists()


def test_copiar_sqlite_consistente_propaga_error_de_origen_inexistente(tmp_path):
    with pytest.raises(sqlite3.OperationalError):
        base_datos.copiar_sqlite_consistente(tmp_path / "inexistente.db", tmp_path / "destino.db")
